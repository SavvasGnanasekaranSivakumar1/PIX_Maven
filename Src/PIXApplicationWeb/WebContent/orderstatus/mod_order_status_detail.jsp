  ...
  <html-el:form action="/orderStatus/orderStatusNew">
    ...
    <td align="left" valign="top" class="padding23">
      ...
      <tr>
         ...
         %> 
          <td height="15" colspan="2" class="text">Following are the details of the <span class="textBold">Status No. <c:out value="${orderStatusForm.statusNo}"/> </span>. To go back to the job status listing, click on the <span class="textBlue">close</span> button or the status tab. To create a new status click on the <span class="textBlue">new status</span> button. </td>
          <%}%>
      	<%
         if("F".equals(request.getParameter("order"))){
         %> 
        <td height="15" colspan="2" class="text">Following are the details of the <span class="textBold">Status No. <c:out value="${orderStatusForm.statusNo}"/> </span>. To go back to the job status listing, click on the <span class="textBlue">close</span> button or the job status tab. </td>
        <%}%>
      </tr>
      <tr>
    ...
    <table width="98%" border="0" cellspacing="1" cellpadding="0">
      <c:set var="errorMsg" value='<%=request.getAttribute(PIXConstants.PIX_ERROR)%>' />
       ...
       <%-- <textarea class="${class1}" readonly="readonly" rows="2" style="background-color:transparent;border:none;height:100%;overflow:hidden; width:250px;text-align:left"> --%>
       </textarea></td>
          </tr>
           </c:when>
           <c:otherwise>
       ...
       <tr>
        ...
        <tr>
         ...
         %> 
          ...
          <input name="Button2" type="button" class="buttonMain" onClick="submitCancelAction('<%=request.getContextPath()%>/orderStatus/orderStatusList.do?PAGE_VALUE=${PAGE_VALUE}&poid=${poid}&poversion=${poversion}&pono=${pono}&rno=${rno}&order=${order}')" value="Close">
