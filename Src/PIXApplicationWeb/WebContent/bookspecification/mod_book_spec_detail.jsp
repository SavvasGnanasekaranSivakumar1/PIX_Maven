...
<c:set var="specnoFilter"><%=request.getParameter("specnoFilter")%></c:set>
...
<c:set var="flipStatusFilter"><%=request.getParameter("flipStatusFilter")%></c:set>
...
<c:forEach var="party" items="${bookspecform.bookSpec.partyCollection}" varStatus="partyCount">
 ...
 <tr>
    ...
    <c:if test="${bookspecform.bookSpec.releaseNo!=null}">
...
<c:forEach var="lineItem" items="${bookspecform.bookSpec.lineItemCollection}" varStatus="lineInfo">
  ...
  <tr>
    ...
    <a name="topofpage"></a>
      <table width="98%" border="0" cellspacing="0" cellpadding="0">
    	...
    	<tr>
      <td class="text">Following are the details of the book specification no. </td>
      </tr>
      ...
      <tr>
        ...
        <table width="100%"  border="0" cellspacing="1" cellpadding="0">
          <tr>
            ...
            <td ></td>
            <td ></td>
            <td height="20" align="right" valign="top" class="subLinksMain">&gt; <a href="javascript:MM_openBrWindow('<%=request.getContextPath()%>/bookspecification/bookspechistorylist.do?SPEC_NO=${bookspecform.bookSpec.specNo}&PAGE_VALUE=1&page=H','BookSpecHistory','scrollbars=yes,width=800,height=500')" class="subLinksMain">history</a> </td>
          </tr>
          <tr>
            ...
            <table width="98%" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td align="left" valign="top"><table width="98%"  border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td width="20%" height="23" class="blueColumn">BUYER COMMENTS: </td>
                      <td height="40" class="lightblueColumn">
                      <textarea class="textsmall" readonly="readonly" rows="5" style="color: black; background-color:transparent; border:none; overflow:visible; width:722px;text-align:left">${buyer.comments}</textarea>
                      </td>
                    </tr>
           	  <tr>
                      <td height="23" class="blueColumn">VENDOR COMMENTS:</td>
	    		...
	    		<html-el:textarea   property="bookSpec.partyCollectionIdx[${partyCounter}].comments" rows="2" style="width:722px" styleClass= "textfield" value="${vendor.comments}" />
	    		<%-- onkeypress="return limitheader(${partyCounter})" --%>
	    	  </c:when>
                  ...
                  <table width="98%" border="0" cellspacing="0" cellpadding="0">
                   ...
                   <tr>
                     ...
                     <table cellpadding="0" cellspacing="0">
          			...
          			<html-el:hidden name="bookspecform" property="module" />
					...
					<%-- <input name="Button" type="button" class="buttonMain" onClick="submitAction('<%=request.getContextPath()%>/bookspecification/bookspecsave.do',this)" value="Accept"> --%>
						<input name="PAGE_VALUE" type="hidden" value="${PAGE_VALUE}">
						<input name="Button" type="button" class="buttonMain" onClick="if(validate(${partyCounter})){submitAction('<%=request.getContextPath()%>/bookspecification/bookspecsave.do',this)}else{return false;}" value="Acknowledge">
					</c:if>
            		<c:if test="${bookspecform.module!=null}">
            		  <input name="Button2" type="button" class="buttonMain" onClick="submitPlanningCancelAction('<%=request.getContextPath()%>/potentialarp/potentialarplist.do?party=V&status=${statusFilter}&PAGE_VALUE=${PAGE_VALUE}&pageFilter=${pageFilter}&flipStatusFilter=${flipStatusFilter}&isbnFilter=${isbnFilter}&printNoFilter=${printNoFilter}&statusFilter=${statusFilter}&startDateFilter=${startDateFilter}&endDateFilter=${endDateFilter}')" value="Close">
            		</c:if>  
            		<c:if test="${bookspecform.module==null}">
                      <input name="Button2" type="button" class="buttonMain" onClick="submitPlanningCancelAction('<%=request.getContextPath()%>/bookspecification/bookspeclist.do?PAGE_VALUE=${PAGE_VALUE}&pageFilter=${pageFilter}&specnoFilter=${specnoFilter}&isbnFilter=${isbnFilter}&printNoFilter=${printNoFilter}&statusFilter=${statusFilter}&startDateFilter=${startDateFilter}&endDateFilter=${endDateFilter}')" value="Close">            		
            		</c:if>
            		
            		<input name="Button2" type="button" class="buttonMain" onClick="submitBspAction('<%=request.getContextPath()%>/pdf/bookspecificationpdf.do',this)" value="Export PDF">
                    </td>
         			</tr>
         			</table>
                    </label></td>
