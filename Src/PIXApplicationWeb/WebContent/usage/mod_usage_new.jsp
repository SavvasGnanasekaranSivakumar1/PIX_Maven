...
<c:set var="usageNo" value="${usageForm.usage.usageNo}"/>
<tr>
 	<c:choose>
		<c:when test="${usageNo != null}">
		...
		<c:otherwise>
			<td height="25" align="left" valign="top"><img src="<%=request.getContextPath()%>/images/heading_icon.gif" width="23" height="9"><span class="headingText">Usage - PO No. ${usageForm.poHeader.poNo} </span></td>
		</c:otherwise>
  	</c:choose>
   <%-- <td height="25" align="left" valign="top"><img src="<%=request.getContextPath()%>/images/heading_icon.gif" width="23" height="9"><span class="headingText">Usage - PO No. ${usageForm.poHeader.poNo}- Usage No.  ${usageForm.usage.usageNo} </span></td> --%>
</tr>
 <c:forEach var="lineItem" items="${usageForm.usage.usageLineCollection}" varStatus="orderLine">
    ...
    <html-el:form action="/usage/newusage">
      ...
      <tr>
          <td height="16" align="left" valign="top"><table width="100%"  border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <td width="2%">&nbsp;</td>
              <td width="30%" valign="top"><fieldset class="legendBorder">
                <legend class="legendeTitle">SHIP TO (V063) </legend>
                <table width="100%" border="0" cellpadding="0" cellspacing="0">
                  <tr>
                    <td height="5"></td>
                  </tr>
        ...
        <c:forEach var="usageLineItem" items="${usageForm.usage.usageLineCollection}" varStatus="lineInfo">
	      ...
	      <tr>
           ...
           <td class="${class1}"><c:out value="${usageLineItem.productDescription}"/></td>
            ...
            <td class="${class1}"><label>
            <html-el:text property="usage.usageLineItemCollectionIdx[${lineInfo.index}].usageQuantity" value="" size="10" styleClass= "textfield"  maxlength="9" onkeypress="return numbersonly()" />
            </label></td>
          <td class="${class1}">  <html-el:text property="usage.usageLineItemCollectionIdx[${lineInfo.index}].damagedQuantity" value="0" size="10" styleClass="textfield" maxlength="9" onkeypress="return numbersonly()" />  </td>
         <td class="${class1}"> <html-el:textarea property="usage.usageLineItemCollectionIdx[${lineInfo.index}].comments" rows="2" value="" styleClass="textfield" style="width:95%" /> </td>
       <!--   <td class="${class1}"> <html-el:textarea onkeypress="return limitChar(${totalLineItems})" property="usage.usageLineItemCollectionIdx[${lineInfo.index}].comments" rows="2" value="" styleClass="textfield" style="width:95%" /> </td> -->
         </tr>
         </c:forEach> 
         </c:if>
          <c:if test="${errorMsg != null}">
      ...
      <table width="98%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          ...
          <td id="buttons2" class="tabSelectTextleft"> 
            ...
            <input name="Button" type="button" class="buttonMain" onClick="if(validate(${totalLineItems})) { submitAction('<%=request.getContextPath()%>/usage/insertusage.do?poid=${poid}&poversion=${poversion}&pono=${pono}&rno=${rno}&uid=${usageForm.usage.usageId}',this) } else {return false;}" value="Save">
            <%--        if(validate(${totalLineItems})){submitAction('<%=request.getContextPath()%>/usage/insertusage.do?poid=${poid}&poversion=${poversion}&pono=${pono}&rno=${rno}&uid=${usageForm.usage.usageId}',this)} else {return false;}    <input name="Button" type="button" class="buttonMain" onClick="submitAction('<%=request.getContextPath()%>/usage/insertusage.do?poid=${poid}&poversion=${poversion}&pono=${pono}&rno=${rno}&uid=${usageForm.usage.usageId}',this)" value="Save"> --%>
            <input name="Button2" type="button" class="buttonMain" onClick="submitCancelAction('<%=request.getContextPath()%>/usage/usageList.do?PAGE_VALUE=${PAGE_VALUE}&ponoFilter=${ponoFilter}&isbnFilter=${isbnFilter}&printNoFilter=${printNoFilter}&startDateFilter=${startDateFilter}&endDateFilter=${endDateFilter}')" value="Close">
           </td>
            </tr>
            </table></label></td>
        </tr>
