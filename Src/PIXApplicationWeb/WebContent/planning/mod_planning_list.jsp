  ...
  <tr>
     ...
     <input type="hidden" name="idHidden" id="hiddenVal" value="${checkBoxIdList}"/>
      ...
      <table width="98%" border="0" cellspacing="0" cellpadding="0">
        ...
        <html-el:link styleClass="subLinksMain" page="/planning/planningfilter.do">filter</html-el:link></td>
        </tr>
        ...
        <%--<table width="100%" border="0" cellspacing="1" cellpadding="0">--%>
          ...
          <td class="${class1}"> 
          <html-el:link styleClass="${class2}" page="/planning/planningdetail.do?poid=${planningpo.poId}&poversion=${planningpo.poVersion}&pono=${planningpo.poNo}&PAGE_VALUE=${PAGE_VALUE}&pageFilter=${pageFilter}&isbnFilter=${isbnFilter}&printNoFilter=${printNoFilter}&statusFilter=${statusFilter}&startDateFilter=${startDateFilter}&endDateFilter=${endDateFilter}&sbBDateFilter=${sbBDateFilter}&ebBDateFilter=${ebBDateFilter}"><c:out value="${planningpo.titleDetail.isbn10}"/></html-el:link>
          <td  width="4px" class="${class1}"><c:out value="${planningpo.titleDetail.printNo}"/></td>
          
		  <td class="${class1}"><fmt:formatNumber pattern="#,###,##0" value="${planningpo.totalQuantity}"></fmt:formatNumber> </td>
      <%--    <td class="${class1}">  <c:out value="${planningpo.totalQuantity}"/></td> --%>
       ...
       <table width="98%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
         ...
         <a href="javascript:listfilter('${PATH}/planning/planninglist.do?PAGE_VALUE=${prevValue}')" class="backNext_append">
          <img src="<%=request.getContextPath()%>/images/prev.gif" alt="previous" border="0"></a>  
           </c:if>
         
           <c:if test="${nextValue!=null && nextValue!=''}">
            <a href="javascript:listfilter('${PATH}/planning/planninglist.do?PAGE_VALUE=${nextValue}')" class="backNext_append">
...
<%}%>       
           <input name="excel"  type="button" class="buttonMain" onClick="javascript:acceptPlanningPOExcelNew('<%=request.getContextPath()%>/exporttoexcel/generateexcel.do?flag=plan')" value="Export to Excel">

            <input name="excel" type="button" class="buttonMain" onClick="submitPlanningActionExcel('<%=request.getContextPath()%>/exporttoexcel/generateexcel.do?flag=planAll',this)"  value="Export All to Excel">
