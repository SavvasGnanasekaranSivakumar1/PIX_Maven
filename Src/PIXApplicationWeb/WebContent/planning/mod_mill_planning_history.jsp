    ...
    <td align="left" valign="top" class="padding23">
     ...
     <%-- <table width="98%" border="0" cellspacing="1" cellpadding="0">--%>
      <c:set var="errorMsg" value='<%=request.getAttribute(PIXConstants.PIX_ERROR)%>'/>
        ...
        <table width="98%" border="0" cellspacing="1" cellpadding="0">
        <tr>
          <td class="tableHeading">PLAN NUMBER </td>	
          <td class="tableHeading">MATERIAL NUMBER </td>
          <td width="4px"class="tableHeading">THIRD LEVEL PAPER</td>
          <td class="tableHeading">QUANTITY</td>
          <td class="tableHeading">ORDER DELIVERY DATE </td>
	      ...
	      <td class="${class1}">
          <html-el:link styleClass="${class2}" page="/planning/millplanninghistorydetail.do?poid=${planningpo.poId}&poversion=${planningpo.poVersion}&pono=${planningpo.poNo}&roleType=M"><c:out value="${planningpo.poNo}"/></html-el:link>
          <td class="${class1}">
          	${planningpo.titleDetail.isbn10}</td>
        ...
        <tr>
          ...
          <html-el:link styleClass="backNext_append"  page="/planning/millplanninghistorylist.do?PAGE_VALUE=${prevValue}&page=H&poid=${poid}&ponumber=${ponumber}">
          <img src="<%=request.getContextPath()%>/images/prev.gif" alt="previous" border="0"></html-el:link>
          </c:if>
          <c:if test="${nextValue!=null && nextValue!=''}">
    	     <html-el:link styleClass="backNext_append"  page="/planning/millplanninghistorylist.do?PAGE_VALUE=${nextValue}&page=H&poid=${poid}&ponumber=${ponumber}">
